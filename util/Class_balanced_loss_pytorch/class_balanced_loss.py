...
def focal_loss(labels, logits, alpha, gamma):
    ...
    BCLoss = F.binary_cross_entropy_with_logits(
        input=logits, target=labels, reduction="none").to(device)
...
def CB_loss(labels, logits, samples_per_cls, no_of_classes, loss_type, beta, gamma):
    ...
    if loss_type == "focal":
        cb_loss = focal_loss(
            labels_one_hot, logits, weights, gamma)
    elif loss_type == "sigmoid":
        cb_loss = F.binary_cross_entropy_with_logits(
            input=logits, target=labels_one_hot, weights=weights).to(device)
    elif loss_type == "softmax":
        pred = logits.softmax(dim=1)
        cb_loss = F.binary_cross_entropy(
            input=pred, target=labels_one_hot, weight=weights).to(device)
    return cb_loss
